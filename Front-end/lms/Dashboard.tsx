import { type ClassData, computeTopPerformers, getRecentAssignments } from './data'
...
export default function Dashboard({ classes, navigate }: { classes: ClassData[]; navigate: (v: View) => void }) {
  ...
  const totalDone = classes.reduce((s, c) => s + c.homework.reduce((s2, h) => s2 + h.completedIds.length, 0), 0)
  const totalHwSlots = classes.reduce((s, c) => s + c.homework.reduce((s2) => s2 + c.students.length, 0), 0)
  const completionPct = totalHwSlots > 0 ? Math.round((totalDone / totalHwSlots) * 100) : 0

  const recent = getRecentAssignments(classes, 5)
  const topPerformers = computeTopPerformers(classes, 3)

  const stats = [
    { label: 'Total Students', value: totalStudents, sub: 'across all classes', color: '#14367D', Icon: IconStudents },
    { label: 'Assigned Courses', value: classes.length, sub: 'active this term', color: '#C0182E', Icon: IconCourses },
    { label: 'Total Assignments', value: totalAssignments, sub: 'published', color: '#0F7B3A', Icon: IconAssignments },
    { label: 'Homework Completion', value: `${completionPct}%`, sub: 'avg. across all classes', color: '#7B2D8B', Icon: IconCheck },
  ]
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 22 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 320px', gap: 18 }}>
        ...
        <div style={card}>
          ...
          <div>
            {recent.map(({ assignment, classLabel, classColor }, i) => (
              <div
                key={assignment.id}
                style={{
                  padding: '13px 20px',
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 11.5, color: '#9CA3AF', marginTop: 2 }}>{classLabel}</div>
                </div>
                <div style={{ fontSize: 11.5, color: '#D1D5DB', flexShrink: 0 }}>{assignment.date}</div>
              </div>
            ))}
          </div>
        </div>
            ...
            {topPerformers.map((p, i) => (
              <div key={p.username} style={{ padding: '13px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
                <div style={{
                  ...
                  background: medalBg[i], color: medalColors[i],
